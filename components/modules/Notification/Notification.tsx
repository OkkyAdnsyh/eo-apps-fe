...
const Notification = () => {
  ...
  return (
    <>
        <Toggle onClick={handleNotifToggle} className={`${statusHasValue ? 'before:block before:w-2 before:h-2 before:bg-red-700 before:rounded-full before:z-10' : ''}`}>
            <FaBell style={{fontSize : '24px', color : "#0369a1"}} />
        </Toggle>
        <Dropdown className={`translate-y-[75%] lg:translate-y-[30%] translate-x-[15%] lg:-translate-x-[90%] w-72 md:w-96 h-auto max-h-40 ${notifIsOpen ? 'scale-y-full' : 'scale-y-0'} origin-top transition-transform duration-500 rounded-xl shadow-lg`}>
            {notif.map((item) => {
                return (
                    <li key={item.id}>
                        <DropItem className='w-full h-20 block'>
                            <Link href={item.url} className={`flex items-center justify-start p-3 text-md lg:text-sm font-medium w-full h-full ${item.status ? 'bg-gray-50 text-gray-500' : 'bg-sky-600 text-gray-100'} backdrop:filter border-b-2 border-b-sky-500/30`} onClick={(e : MouseEvent<HTMLAnchorElement>) => {
